...
# Script to generate MAKE Phrasal List pages (phrasal/xx/index.html)

base_dir = r'c:\Users\yuto\OneDrive\Toeic-cards--main\verbs\make\phrasal'
output_base_dir = r'c:\Users\yuto\OneDrive\Toeic-cards--main\verbs\make\phrasal'

def generate_phrasal_list(group_id):
    group_str = f"{group_id:02d}"
    source_path = os.path.join(base_dir, f'phrasal{group_str}の原文テキスト.txt')
    ...
    for line in lines:
        ...
        # Parse phrases: make up - translation
        match = re.match(r'^(make\s+.*?)\s-\s(.*)$', line)
        if match:
            phrases.append({
    ...
    # Define Description text based on group (Hardcoded just for the header desciption if needed, or generic)
    header_description = "最も重要な基本フレーズ。" # Default
    if group_id == 1:
        header_description = "make up / make out / make for... 最も重要な基本フレーズ。"
    elif group_id == 2:
        header_description = "make up for / make fun of... 表現力を広げる応用フレーズ。"
    elif group_id == 3:
        header_description = "make money / make a difference... ビジネスで必須のコロケーション。"
    elif group_id == 4:
        header_description = "make it possible / make sure... 複雑な構文を作る重要表現。"
...
<head>
    ...
    <title>make 句動詞 {group_str}｜{subtitle}</title>
        ...
        <header class="space-y-4 border-b border-orange-100 pb-6">
            ...
            <div class="space-y-2">
                <div
                    ...
                    GROUP {group_str}</div>
                <h1 class="text-3xl md:text-4xl font-black text-slate-900">make 句動詞 {group_str}｜{subtitle}</h1>
